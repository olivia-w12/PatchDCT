...
class MaskRCNNDCTHead(BaseMaskRCNNHead):
    ...
    def __init__(self, input_shape: ShapeSpec, *, num_classes, dct_vector_dim, mask_size,
                 dct_loss_type, mask_loss_para,pooler_resolution,
        ...
        self.conv2 = nn.Conv2d(256,300,kernel_size=1,stride=1)
        self.linear = nn.Linear(self.pooler_resolution*self.pooler_resolution,1)
    ...
    def layers(self, x):
        ...
        x=x.reshape(-1,self.dct_vector_dim)
        return x
    ...
    def mask_rcnn_dct_loss(self, pred_mask_logits,  instances, vis_period=0):
        ...
        if self.dct_loss_type == "l1":
            ...
            mask_loss= F.l1_loss(pred_mask_logits, gt_masks, reduction="sum")
