...
class MaskRCNNPatchDCTHead_refine_DCT_vectors(BaseMaskRCNNHead):
    """
    ...
    def __init__(self, input_shape: ShapeSpec, *, num_classes, dct_vector_dim, mask_size,
        ...
        self.predictor2 = nn.Sequential(
            nn.Linear(fine_features_resolution ** 2 * conv_dim, self.hidden_features),
            nn.ReLU(),
            nn.Linear(self.hidden_features, self.hidden_features),
            nn.ReLU(),
            nn.Linear(self.hidden_features, self.dct_vector_dim)
    ...
    def layers(self, x,fine_mask_features):
        ...
        x2 = self.predictor2(fine_mask_features.flatten(start_dim=1))
    ...
    def forward(self, x, fine_mask_features,instances: List[Instances]):
        ...
        """
        x,x2 = self.layers(x,fine_mask_features,)
        if self.training:
            return {"loss_mask": self.mask_rcnn_dct_loss(x,x2,instances, self.vis_period)}
        else:
            pred_instances = self.mask_rcnn_dct_inference(x,x2,instances)
    ...
    def mask_rcnn_dct_loss(self, pred_mask_logits, pred_mask_logits2,instances, vis_period=0):
        ...
        """
        gt_masks_coarse= self.get_gt_mask(instances,pred_mask_logits)


        if self.dct_loss_type == "l1":
            mask_loss_1 = F.l1_loss(pred_mask_logits, gt_masks_coarse)
            mask_loss_2 = F.l1_loss(pred_mask_logits2,gt_masks_coarse)
    ...
    def mask_rcnn_dct_inference(self,pred_mask_logits,pred_mask_logits2,pred_instances):
        """
        ...
        if num_patch == 0:
            ...
            pred_mask_rc = self.dct_encoding_coarse.decode(pred_mask_logits2.detach())
