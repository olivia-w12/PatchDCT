...
class MaskRCNNPatchDCTHead_deconv(BaseMaskRCNNHead):
    """
    ...
    def __init__(self, input_shape: ShapeSpec, *, num_classes, dct_vector_dim, mask_size,
        ...
        self.scale = self.mask_size_assemble // self.patch_size
        ...
        self.predictor1 = Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def layers(self, x,fine_mask_features):
        ...
        fine_mask_features = self.deconv(fine_mask_features)
    ...
    def forward(self, x, fine_mask_features,instances: List[Instances]):
        ...
        """
        x,detail = self.layers(x,fine_mask_features,)
        if self.training:
            return {"loss_mask": self.mask_rcnn_dct_loss(x,detail,instances, self.vis_period)}
        else:
            pred_instances = self.mask_rcnn_dct_inference(x,detail,instances)
            return pred_instances

    def mask_rcnn_dct_loss(self, pred_mask_logits,detail,instances, vis_period=0):
        """
        ...
        """
        gt_classes,gt_masks_coarse,gt_masks_vector= self.get_gt_mask(instances,pred_mask_logits)
        num_instance = gt_classes.size()[0]
        indice = torch.arange(num_instance)

        detail = detail[indice,gt_classes]
        # detail = detail[:,None,:,:]
        # detail = F.interpolate(detail,scale_factor=self.patch_size,mode="bilinear")

        if self.dct_loss_type == "l1":
            mask_loss_1 = F.l1_loss(pred_mask_logits, gt_masks_vector)
            mask_loss_2 = F.binary_cross_entropy(torch.sigmoid(detail),gt_masks_coarse)
    ...
    def mask_rcnn_dct_inference(self,pred_mask_logits,detail,pred_instances):
        """
        ...
        if num_patch == 0:
            ...
            detail = detail[indices,pred_classes]
            ...
            with torch.no_grad():
                ...
                pred_mask_rc = detail[:,None,:,:]
                # pred_mask_rc = F.interpolate(detail,scale_factor=self.patch_size,mode="bilinear")
    ...
    def get_gt_mask(self,instances,pred_mask_logits):
        ...
        gt_masks_coarse = gt_masks_coarse.to(dtype=torch.float32)
        gt_masks_vector = self.dct_encoding_coarse.encode(gt_masks_coarse)
        return gt_classes,gt_masks_coarse,gt_masks_vector
