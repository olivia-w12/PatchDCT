...
class ROIHeads_3_stage(ROIHeads):
    """
    ...
    def __init__(
        ...
        *,
        ...
        fine_features2: Optional[List[str]] = None,
        fine_mask_pooler2: Optional[ROIPooler] = None,
        ...
        **kwargs
        ...
        if self.mask_on:
            ...
            self.fine_mask_pooler2 = fine_mask_pooler2
            self.fine_features2 = fine_features2
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if inspect.ismethod(cls._init_mask_head):
            ...
            ret.update(cls._init_fine_mask_head2(cfg, input_shape))
    ...
    @classmethod
    def _init_fine_mask_head2(cls, cfg, input_shape):
        if not cfg.MODEL.MASK_ON:
            return {}
        # fmt: off
        in_features       = cfg.MODEL.ROI_MASK_HEAD.FINE_FEATURES2

        pooler_resolution = cfg.MODEL.ROI_MASK_HEAD.FINE_FEATURES_RESOLUTION2
        pooler_scales     = tuple(1.0 / input_shape[k].stride for k in in_features)
        sampling_ratio    = cfg.MODEL.ROI_MASK_HEAD.POOLER_SAMPLING_RATIO
        pooler_type       = cfg.MODEL.ROI_MASK_HEAD.POOLER_TYPE
        # fmt: on
        fine_features = cfg.MODEL.ROI_MASK_HEAD.FINE_FEATURES2
        in_channels = [input_shape[f].channels for f in in_features][0]

        ret = {"fine_features2":fine_features}
        ret["fine_mask_pooler2"] = ROIPooler(
            output_size=pooler_resolution,
            scales=pooler_scales,
            sampling_ratio=sampling_ratio,
            pooler_type=pooler_type,
    ...
    def _forward_mask(
        ...
        fine_features_layers2 = [features[f] for f in self.fine_features2]
        ...
        if self.training:
            ...
            fine_mask_features2 = self.fine_mask_pooler2(fine_features_layers2, proposal_boxes)
            return self.mask_head(mask_features,fine_mask_features,fine_mask_features2,proposals)
        else:
            pred_boxes = [x.pred_boxes for x in instances]
            mask_features = self.mask_pooler(features, pred_boxes)
            fine_mask_features = self.fine_mask_pooler(fine_features_layers, pred_boxes)
            fine_mask_features2 = self.fine_mask_pooler2(fine_features_layers2, pred_boxes)
            return self.mask_head(mask_features,fine_mask_features,fine_mask_features2,instances)
