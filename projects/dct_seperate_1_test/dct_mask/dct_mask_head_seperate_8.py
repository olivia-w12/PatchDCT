...
class MaskRCNNDCTHead8(BaseMaskRCNNHead):
    ...
    def forward(self, x, fine_mask_features,instances: List[Instances]):
        ...
        """
        x,bfg,detail = self.layers(x,fine_mask_features,)
        if self.training:
            return {"loss_mask": self.mask_rcnn_dct_loss(x,bfg,detail,instances, self.vis_period)}
        else:
            pred_instances = self.mask_rcnn_dct_inference(x,bfg,detail,instances)
            return pred_instances

    def mask_rcnn_dct_loss(self, pred_mask_logits,bfg,detail,instances, vis_period=0):
        """
